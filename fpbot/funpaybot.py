...
class FunPayBot:
    ...
    def send_message(self, chat_id: int | str, text: Optional[str] = None, chat_name: Optional[str] = None,
        ...
        if not text and not image_id:
            return None
        # Добавляем watermark один раз до цикла повторных попыток.
        # Иначе при временной ошибке отправки подпись добавляется повторно.
        if (
            text
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # FunPay может временно отклонить два сообщения, отправленных
                # подряд в один чат. В старой версии бот сразу делал три
                # повторные попытки, поэтому все три попадали в тот же лимит.
                error_text = getattr(e, "error_message", None) or ""
                logger.warning(
                    f"{Fore.LIGHTYELLOW_EX}FunPay отклонил сообщение в чат {Fore.WHITE}{chat_id}"
                    f"{Fore.LIGHTYELLOW_EX}: {Fore.WHITE}{error_text}"
                )
                if attempt + 1 < max_attempts:
                    delay = 3.0 if error_text else 1.0
                    time.sleep(delay)
                    continue
                break
            except RequestFailedError as e:
                logger.warning(
                    f"{Fore.LIGHTYELLOW_EX}Ошибка запроса при отправке сообщения в чат {Fore.WHITE}{chat_id}"
                    f"{Fore.LIGHTYELLOW_EX}: {Fore.WHITE}{e.short_str() if hasattr(e, 'short_str') else e}"
                )
                if attempt + 1 < max_attempts:
                    time.sleep(1.0)
                    continue
                ...
                logger.error(
                    f"{Fore.LIGHTRED_EX}Ошибка при отправке сообщения {Fore.WHITE}«{text_for_log}» "
                    f"{Fore.LIGHTRED_EX}в чат {Fore.WHITE}{chat_id} {Fore.LIGHTRED_EX}: {Fore.WHITE}{e}"
                )
                return None

        text_for_log = text.replace('\n', ' ').strip()
        logger.error(
            f"{Fore.LIGHTRED_EX}Не удалось отправить сообщение {Fore.WHITE}«{text_for_log}» "
            f"{Fore.LIGHTRED_EX}в чат {Fore.WHITE}{chat_id} {Fore.LIGHTRED_EX}после {max_attempts} попыток"
        )
        return None
    ...
    async def _on_new_message(self, event: NewMessageEvent):
        # Apple TopUp handles NEW_MESSAGE before the core handler.
        # Do not run the legacy core message logic a second time.
        if getattr(event, "_apple_topup_handled", False):
            return
    ...
    async def run_bot(self):
        ...
        # Module handlers are connected before the FunPay bot starts.
        # Put the core NEW_MESSAGE handler after them so Apple TopUp can
        # claim its message before the legacy menu/custom-command logic.
        new_message_handlers = get_funpay_event_handlers().get(EventTypes.NEW_MESSAGE, [])
        add_funpay_event_handler(
            EventTypes.NEW_MESSAGE,
            FunPayBot._on_new_message,
            len(new_message_handlers),
        )
